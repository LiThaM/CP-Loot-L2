...
<x-mail.layout :kicker="'Ticket ' . $number">
    ...
    <p style="margin:0 0 14px;">Tu ticket <strong>{{ $number }}</strong> «{{ $ticket->subject }}» tiene una respuesta nueva de {{ $replier?->name ?? 'nuestro equipo' }}.</p>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#0b1220;border:1px solid #1f2937;border-radius:12px;padding:14px 16px;margin-bottom:18px;color:#d1d5db;font-size:13px;">
        <tr><td style="padding:3px 0;color:#9ca3af;width:90px;">Quien responde</td><td style="padding:3px 0;">{{ $replier?->name ?? '—' }}</td></tr>
        <tr><td style="padding:3px 0;color:#9ca3af;">Cuándo</td><td style="padding:3px 0;">{{ optional($reply->created_at)->format('Y-m-d H:i') }}</td></tr>
    </table>

    <div style="font-size:11px;font-weight:700;letter-spacing:2px;color:#9ca3af;text-transform:uppercase;margin-bottom:8px;">Respuesta</div>
