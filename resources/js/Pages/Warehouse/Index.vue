...
const myItemsTotal = computed(() => (props.myItems || []).length);
const myWarehouseFilter = ref('');

const filteredMyItems = computed(() => {
    const items = props.myItems || [];
    const q = myWarehouseFilter.value.trim().toLowerCase();
    if (!q) return items;
...
const openReturn = (item) => {
    selectedMyItem.value = item;
    returnForm.item_id = item.id;
    returnForm.amount = 1;
    returnForm.image_proof = null;
    returnModalOpen.value = true;
};

const submitReturn = () => {
    returnForm.post(route('warehouse.return'), {
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
            ...
            <div class="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg border border-gray-200 bg-gray-100 flex items-center justify-center overflow-hidden dark:border-gray-700 dark:bg-black/40">
                        <img v-if="selectedMyItem?.image_url" :src="selectedMyItem.image_url" class="w-full h-full object-cover">
                    </div>
                    <div>
