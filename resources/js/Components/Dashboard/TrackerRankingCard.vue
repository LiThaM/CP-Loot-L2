    ...
    <div class="l2-panel p-5 rounded-lg border border-emerald-500/15 bg-gradient-to-b from-emerald-500/5 to-transparent backdrop-blur">
        ...
        <div v-else class="space-y-2">
            <div v-for="(m, idx) in ranking.slice(0, 5)" :key="m.id"
                 ...
                 :class="isMe(m.id)
                     ? 'border-amber-400/70 bg-amber-50/80 ring-1 ring-amber-400/50 dark:border-amber-400/40 dark:bg-amber-900/20'
                     : 'border-gray-200 bg-white/70 dark:border-white/5 dark:bg-black/20'">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <div class="text-[11px] font-black truncate" :class="isMe(m.id) ? 'text-amber-700 dark:text-amber-300' : 'text-gray-900 dark:text-white'">
                            {{ m.name }}<span v-if="isMe(m.id)" class="ml-1 text-[9px] font-black uppercase">· {{ tr('tú', 'you') }}</span>
                        </div>
